...
Strange but (I think) true: More possessions have ended with a `d_line` on offense (445 `d_line` vs. 397 `o_line`). In fact, *more goals* have been scored by `d_lines` than `o_lines` (184 by `d_lines` vs. 178 by `o_lines`). OK those are actually very close, but *a priori* I would have expected `o_lines` to account for more goals. This could lead to a conjecture that ... if there's parity in the Conference re: efficiency of `o_lines`, then the ability of `d_lines` to play offense is what gives a team an edge. Or maybe if I address line changes this all goes away ...

This plot does confirm the `o_lines` on offense are more likely to score, less likely to turn it over through error, and more likely to get D'd than `d_lines`. Conversely, `d_lines` are as likely to turn it over as they are to score -- but they don't have to worry so much about getting D'd by an `o_line` playing defense.
...
To be clear here, within a team * line combination, the bar heights add up to 1. So we get the breakdown of how possessions end when, e.g. a Portland Stags' `o_line` had the disc: 56% end in a goal for them, 24% end in a turnover nominally due to offense error, and 19% end with an explicit stop by the defense.
