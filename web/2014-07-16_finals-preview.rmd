...
tmp <- ddply(pl_stats_agg, ~ team, function(x) {
  x <- subset(x, select = c(pl_long, goals, team, last))
  tot_goals <- sum(x$goals)
  x <- subset(x, goals > 0)
  x$pl_long <- with(x, reorder(pl_long, goals))
  x$goal_share <- x$goals/tot_goals
  x$last <-with(x, ifelse(goal_share > 0.05, last, ''))
  x <- subset(x, rank(-goal_share) <= 15)
  return(x)
  })
#str(tmp)
p <- ggplot(tmp, aes(x = pl_long, y = goal_share, fill = team))
...
tmp <- ddply(pl_stats_agg, ~ team, function(x) {
  x <- subset(x, select = c(pl_long, def, team, last))
  tot_ds <- sum(x$def)
  x <- subset(x, def > 0)
  x$pl_long <- with(x, reorder(pl_long, def))
  x$def_share <- x$def/tot_ds
  x$last <-with(x, ifelse(def_share > 0.06, last, ''))
  x <- subset(x, rank(-def_share) <= 15)
  return(x)
  })
#str(tmp)
