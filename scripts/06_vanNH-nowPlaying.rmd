...
sDat <- transform(sDat, recvNum = jFun(recvNum), pullNum = jFun(pullNum))

## determine 'master' code for an event
## assumes only one of pullCode and recvCode is populated, which is dangerous FIX
...
sDat <- subset(sDat, code %in% c('D', 'HB', 'FB', 'A', 'LA', 'PUA', 'G', 'LG'))
#str(sDat)
#table(sDat$code)
...
plDat <- ddply(sDat, ~ player + plTeam, function(z) {
  goals <- sum(z$code %in% c("G", "LG"))
  assists <- sum(z$code %in% c("A", "LA", "PUA"))
  Ds  <-  sum(z$code %in% c("D", "HB", "FB"))
...
gDat <- ddply(sDat, ~ point,
              function(z) {
                ...
                scorer <- paste(z$player[z$code %in% c("G", "LG")],
                                z$lastName[z$code %in% c("G", "LG")])
                assist <- z$code %in% c("A", "LA", "PUA")
                assister <- paste(z$player[z$code %in% c("A", "LA", "PUA")],
                                  z$lastName[z$code %in% c("A", "LA", "PUA")])
