...
tmp <- subset(pl_stats_agg, goals > 0)
tmp$pl_long <- with(tmp, reorder(pl_long, goals))
p <- ggplot(tmp, aes(x = pl_long, y = goals / tot_goals))
...
tmp <- subset(pl_stats_agg, def > 0)
tmp$pl_long <- with(tmp, reorder(pl_long, def))
p <- ggplot(tmp, aes(x = pl_long, y = def / tot_ds))
p + geom_bar(stat = "identity") + coord_flip()
```
...
p <- ggplot(tmp, aes(x = def / tot_ds, y = goals / tot_goals))
