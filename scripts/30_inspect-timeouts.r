## a one-off script to look at the 5 rows surrounding all timeouts
...
jFun <- function(x) grepl("to", x, ignore.case = TRUE)
is_to <- with(sDat, which(jFun(pullCode) | jFun(recvCode)))
rows_i_want <- sort(unique(unlist(lapply(is_to,
                                         function(x)
                                           seq(from = x - 2, to = x + 2)))))
