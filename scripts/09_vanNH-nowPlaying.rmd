...
## DELETE THIS ONCE I'M DONE CHECKING OLD VS NEW PLAYER STATS
## check pull_team in game_play against point_info
# point_info$pull_team
# with(sDat, pull_team[event == 1])
# data.frame(pi = point_info$pull_team, sd = with(sDat, pull_team[event == 1]),
#            point_info$pull_team == with(sDat, pull_team[event == 1]))
...
jFun <- function(z) {
  ...
  time_before_after <-
    unlist(point_info[point_info$point == pt,
                      grep("clk", names(point_info))])
  names(time_before_after) <- NULL
  time_NA <- is.na(time_before_after)
  if(any(time_NA)) {
    time_before_after[time_NA] <- point_duration_text <- "???"
    message("  ALERT point is missing time before and/or after:")
    message("        point: ", pt, " clk_before: ",
            time_before_after["clk_before"], " clk_after: ",
            time_before_after["clk_after"])
    } else {
      point_duration <-
